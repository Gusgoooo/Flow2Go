...
export function EditableSmoothStepEdge(props: EdgeProps) {
  ...
  const points = useMemo((): Point[] => {
    // For generated edges, enforce a minimal out/in terminal lead to avoid zero-length terminal segments.
    // Keep manual-edited edges flexible (lead=0) so users can still merge parallel segments when desired.
    const isGeneratedEdge =
      Boolean(dataTyped.autoGeneratedSwimlane) ||
      Boolean(dataTyped.layoutProfile) ||
      Boolean(dataTyped.labelTextOnly)
    const minTerminalLead = isGeneratedEdge ? EDGE_STEP_UNIT : 0
